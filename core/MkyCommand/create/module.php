...
if (php_sapi_name() === "cli") {
    ...
    foreach ($files as $key => $file) {
        $template = file_exists(__DIR__ . "/templates/$option/$key." . MickyCLI::EXTENSION) ? file_get_contents(__DIR__ . "/templates/$option/$key." . MickyCLI::EXTENSION) : '';
    ...
    $template = file_get_contents(__DIR__ . "/templates/$option/module." . MickyCLI::EXTENSION);
    ...
    foreach (['Controllers' => "{$name}Controller", 'Controllers\\Admin' => "Admin/{$name}Controller"] as $namespace => $file) {
        $template = file_get_contents(__DIR__ . "/templates/controller." . MickyCLI::EXTENSION);
    ...
    $moduleServiceProviderFile = getcwd() . "/app/Providers/ModuleServiceProvider.php";
    $arr = explode("\n", file_get_contents($moduleServiceProviderFile));
    $moduleLine = array_keys(preg_grep("/return \[/i", $arr))[0];
    array_splice($arr, $moduleLine + 1, 0, "\t\\App\\$name\\{$name}Module::class,");
    $arr = array_values($arr);
    $arr = implode("\n", $arr);
    $ptr = fopen($moduleServiceProviderFile, "w");
    fwrite($ptr, $arr);
